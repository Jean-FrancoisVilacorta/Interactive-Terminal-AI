...
static void black_info(struct line_h *data)
{
    int len = data->len;
    char spaces[len];

    printf("%s~%s%s", BLACK, data->path, RESET);
    if (data->git == true) {
        printf(" %sgit:(%s)%s",
            BLACK, data->branch, RESET);
        len = len - 6 - strlen(data->branch);
    }
    len = len - strlen(data->path + 1) - strlen(data->time) - 3;
    spaces[0] = '\0';
...
void last_print(struct line_h *data, struct history_t *buff)
{
    ...
    autocomplete_lines(data, buff);
    lines = GET_LINES_NEEDED(len, data->len);
    remove_lines(lines + data->auto_lines + 1);
    black_info(data);
}
...
static void print_autocomplete(struct line_h *data, size_t lines)
{
    ...
    for (size_t i = 0; i != max_height; i++) {
        for (size_t a = 0; a != how_many_str && file != NULL; a++) {
            print_auto(file, max_size, x, data->column);
            file = file->next;
            x++;
        }
        if (file == NULL) {
